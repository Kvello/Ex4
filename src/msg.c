...
int msg_send_message(int socket, struct StopAndWaitMessage* msg, struct sockaddr *address){
    ...
    */
    ...
    memcpy(&buf,&(msg->header),MSG_HEADER_SIZE); // copy message header to buffer
    memcpy(&buf[MSG_HEADER_SIZE],msg->data,msg->header.data_size); // same as above
    socklen_t addr_size = sizeof(*address);
    int ret = udp_tx(buf, MSG_HEADER_SIZE + msg->header.data_size, socket, address, addr_size);
    return ret;
}
int msg_receive_message(const int sock,struct StopAndWaitMessage* recv,struct sockaddr* src_addr, socklen_t addr_size){
    ...
    */
    ...
    if (ret < (int)MSG_HEADER_SIZE){
        printf("message too short\n");
        return -1;
    }else if(ret > (int)MSG_HEADER_SIZE+recv->header.data_size){
